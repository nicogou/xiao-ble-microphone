 ...
 *
 * Two-thread design:
 *   rec_thread  – reads PDM blocks via the shared mic layer and copies them
 *                 into a ring buffer (high priority, never stalls).
 *   write_thread – drains the ring buffer and writes to a FAT file (lower
 *                 priority, may stall on flash erases or USB-MSC contention).
 *
 * Shell commands:
 *   record start [max_seconds]  – start recording to recNNNN.wav
 *   record stop                 – stop and write the WAV header
...
LOG_MODULE_REGISTER(rec, CONFIG_APP_LOG_LEVEL);
 ...
 * Ring buffer
 *
 * 8 slots × 3200 B ≈ 800 ms – absorbs flash-write and USB-MSC latency
 * spikes.  Full-ring blocks are dropped with a warning; the recording
 * stays alive.
 * ========================================================================= */
...
static atomic_t  rec_bytes        = ATOMIC_INIT(0);
static atomic_t  rec_bytes_queued = ATOMIC_INIT(0);
 ...
 * PDM block callback
 *
 * Clamps the block to the max-duration limit, copies it into the ring
 * buffer, and posts it to the FAT writer.  K_NO_WAIT on the semaphore
 * means a full ring causes a drop rather than blocking the PDM path.
 * ========================================================================= */
 ...
 * FAT writer thread
 *
 * Drains the ring buffer and writes to the open FAT file.  Flash erases
 * and USB-MSC mutex contention only stall this thread; the PDM reader is
 * unaffected.
 * ========================================================================= */
...
static void write_thread_fn(void *p1, void *p2, void *p3)
{
	...
	/* Set false on the first write error to stop hammering fs_write while
	 * still draining the ring so semaphore slots are returned. */
	bool write_ok = true;
	...
	while (true) {
		...
		if (!msg.buf) {
			break;  /* NULL sentinel = PDM reader finished */
		}
		...
		if (write_ok) {
			...
			if (written == (ssize_t)msg.size) {
				...
				LOG_WRN("Storage full (%zd/%u B written) - stopping recording",
					written, msg.size);
				atomic_set(&rec_stop_req, 1);
				write_ok = false;
			}
	...
	/* Drain leftovers after a write error */
	{
		struct ring_msg msg;
	...
	/* Truncate to actual recorded size, freeing unused pre-allocated clusters */
	{
		...
		if (tret < 0) {
			LOG_WRN("Truncate failed (%d)", tret);
		}
	}
...
static int cmd_record_start(const struct shell *sh, size_t argc, char **argv)
{
	...
	/* Write a zeroed placeholder header; replaced with the real one on stop. */
